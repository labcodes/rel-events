...
export class Event {
  constructor({ name, manager, listenTo = [] } = {}) {
    ...
    if (
      ...
      || !listenTo.every(
        obj => obj.hasOwnProperty('event') && obj.hasOwnProperty('trigger') && typeof obj.event === 'function'
      )
    ) {
      throw new Error('ListenTo must be an array of { event, trigger } objects, and the event key sould be a function that returns an Event of RequestEvent.');
    }
  ...
  createReducers = () => {
    ...
    reducers[this.name] = (state = this.manager.initialState, action) => {
      ...
      if (action.type === this.reducers.request) {
        newState = this.manager.onRequest(state, action);
        return newState;
      }
